...
const STORAGE_KEY = 'appleLinkContext';
...
export default function AppleLinkAccount() {
  ...
  const appleEmail = initialContext?.email || '';

  const [email, setEmail] = useState(appleEmail);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!linkToken) {
      setErrorMessage('Missing Apple link request. Please try signing in with Apple again.');
      return;
    }
    ...
    setSubmitting(true);
    try {
      const response = await axios.post('/api/auth/apple/confirm-link', {
        linkToken,
      ...
      setErrorMessage(serverMessage || error?.message || 'Failed to link Apple sign-in.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Container maxWidth="sm" sx={{ py: 6 }}>
      <Typography variant="h5" component="h1" gutterBottom>
        Link Sign in with Apple
      </Typography>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        An account already exists for this email. Confirm your email and password to link Sign in with Apple to your existing account.
      </Typography>

      {!linkToken && (
        <Alert severity="error" sx={{ mb: 3 }}>
          Missing Apple link request. Please go back and try signing in with Apple again.
        </Alert>
      )}
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <TextField
          ...
          autoComplete="email"
          disabled={submitting}
          helperText={appleEmail ? `Apple returned: ${appleEmail}` : undefined}
        />
        <TextField
          label="Password"
          type="password"
